...
Cellular automata (and CGoL itself) have been used to do many things. To list a few: simulating biological systems and physical systems (e.g. [gas models](https://en.wikipedia.org/wiki/Lattice_gas_automaton)), calculating primes ([one early design by me](https://conwaylife.com/forums/viewtopic.php?f=11&t=3233&p=55338&hilit=prime#p55338) and [a much better design](https://conwaylife.com/wiki/Primer) by someone else), simulating itself (see [metacell](https://conwaylife.com/wiki/Unit_cell)). No wonder Wikipedia calls it a "[model of computation](https://en.wikipedia.org/wiki/Cellular_automaton#:~:text=is%20a%20discrete-,model%20of%20computation,-studied%20in%20automata)"! 

As you can see, people have made great interesting things with cellular automata. But note that that ones I've listed were all intentionally designed, be it patterns or the actually update rules. I personally like waste my free time on what the community calls *rule-golfing*, i.e. exploring the rule space by random walk. If you relax CGoL's update rule from count of ON states in its neighbor to their exactly arrangement, while maintaining rotational and reflectional symmetry, then you get about 2^102≈5.07e30 different rules. Inside this giant rule space, I usually start with someone else's interesting rule, then make random changes to it until I observe interesting behavior in the evolved configurations. The rules and objects I end up with are really discovered rather than constructed.

Here I list the particularly cool-looking, unusual, or otherwise notable discoveries by me. The gibberish text encoding the rule and the pattern in [RLE format](https://conwaylife.com/wiki/Run_Length_Encoded), which you can simulate yourself using e.g. [Golly](https://conwaylife.com/wiki/Tutorials/Golly#RLEs_and_pattern_files).
